...
Platform 使用 **后端菜单驱动模式**（`accessMode: 'backend'`），路由和菜单由后端动态返回。
...
│       └── monitor.ts           # Monitor management routes
└── plugin/
...
```typescript
// apps/platform/src/router/routes/modules/system.ts
...
```typescript
// apps/platform/src/router/plugin/scanner.ts
export function scanPluginRoutes() {
  // Scan plugins/{plugin}/resources/assets/platform/routes/
...
```typescript
// plugins/{plugin}/resources/assets/platform/routes/index.ts
export default {
  path: '/plugin/{plugin}',
  name: 'PlatformPlugin{Name}List',
...
```typescript
// apps/platform/src/router/guard.ts
