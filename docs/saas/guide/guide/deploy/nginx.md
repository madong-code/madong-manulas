...
Nginx 在 80/443 统一收口，把 API 反代到后端 Webman（8500），静态资源直接托管。以下为生产参考配置（基于 `template/mono/apps/admin/build/deploy/nginx.conf` 改造）。
...
server {
    ...
    location /admin/ {
        alias /var/www/mdadmin/template/mono/apps/admin/dist/;
        try_files $uri $uri/ /admin/index.html;
        index index.html;
    }

    # 平台静态站（也可由 Webman 直接服务 public/platform）
    location /platform/ {
        alias /var/www/mdadmin/template/mono/apps/platform/dist/;
    ...
    # 安装向导
    location /install/ {
        alias /var/www/mdadmin/template/mono/apps/install/dist/;
