...
admin / platform / install 三个前端都在 `template/mono/`（pnpm monorepo）下：

```bash
cd template/mono

# 1. 安装依赖（必须用 pnpm，一次覆盖三个应用）
pnpm install
...
访问 `http://localhost:5777`（端口由 `template/mono/apps/admin/.env.development` 的 `VITE_PORT` 决定）。
...
`template/mono/apps/admin/.env`（通用）：
...
`template/mono/apps/admin/.env.development`（开发环境）：
