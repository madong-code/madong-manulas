...
- **admin UI 替换**：`template/admin` 是固定位置、可整体换 UI 的模版，靠 `package.json` 的 `name` 区分当前 UI（如 `vue-vben-admin-ele` / `madong-vue`）。替换 UI 时**后端契约不变**，只需保证新模版调用相同 API。
...
- 数据库（全量 dump）。
- `backend/.env`（配置）。
- `public/admin`、`public/web`、`public/install` 构建产物（可重新构建，建议保留旧版本以便回滚）。
