...
├── admin/                      # 后台前端模板
│   ├── views/                  # 页面（与 template/mono/apps/admin/src/views 一致结构）
│   ├── lang/                   # i18n 文案（json）
...
- `lang/*.json` 为多语言文案，安装时合并进 `template/mono/apps/admin/src/lang/`（或对应模块）。
...
- 安装插件：复制 `resource/template/admin/**` → `template/mono/apps/admin/src/**`。
